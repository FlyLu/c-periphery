...
For a blocking read with the VMIN setting configured, `serial_read()` will return when at least VMIN bytes are read. For a blocking read with both VMIN and VTIME settings configured, `serial_read()` will return when at least VMIN bytes are read or when the VTIME interbyte timeout expires after the last byte read. In both cases, `serial_read()` may return less than the requested number of bytes.
...
`serial` should be a valid pointer to a Serial handle opened with `serial_open()` or `serial_open_advanced()`. `timeout_ms` can be positive for a timeout in milliseconds, zero for a non-blocking poll, or a negative number for a blocking poll.
